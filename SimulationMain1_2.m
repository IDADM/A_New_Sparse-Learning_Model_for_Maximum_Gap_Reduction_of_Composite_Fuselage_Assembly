...
rng('default'); % set up the same seed (random number generation)
...
rho1=norm(betas-betaE)/(sqrt(sigma2*S*log(p)))
...
%% Evaluation index caculation 
rho1R=norm(betas-betaES)/(sqrt(sigma2*S*log(p)))
%% The result is saved in 'SimulationMain1_2R.mat'
%% visualization 
